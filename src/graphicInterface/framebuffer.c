...
int xRightWindowOffset,xLeftWindowOffset, yTopWindowOffset, yBottomWindowOffset;
unsigned char *fb;
...
void setYWindowOffsets(int yOffset, yOffsetLocation offsetLocation){
    if(offsetLocation == BOTTOM){
        yBottomWindowOffset = yOffset;
    }
    else{
        yTopWindowOffset = yOffset;
    }
}

void setXWindowOffsets(int xOffset, leftRight offsetLocation){
    if(offsetLocation == LEFT){
        xLeftWindowOffset = xOffset;
    }
    else{
        xRightWindowOffset = xOffset;
    }
}

unsigned int getWindowHeight(){
    return getScreenHeight() - (yTopWindowOffset * sizeScale) - (yBottomWindowOffset * sizeScale);
}

unsigned int getWindowWidth(){
    return getScreenWidth() - (xRightWindowOffset * sizeScale) - (xLeftWindowOffset * sizeScale);
}
...
void drawScaledPixelsWindow(int x, int y, int color){
    x += xLeftWindowOffset;
    y += yTopWindowOffset;
    drawScaledPixelsScreen(x, y, color);
}
        ...
        for(int y = 0; y < height; y++){
            switch((y/10)%5){
                ...
                case 1:
                    ...
                    break;
                case 2:
                    drawScaledPixelsWindow(x,y,green);
                    break;
                case 3:
                    drawScaledPixelsWindow(x,y,red);
                    break;
                case 4:
                    drawScaledPixelsWindow(x,y,blue);
                    break;
            }
        }
    }
...
void setInterfaceScaling(unsigned int scalingValue){
    ...
    for(int y = 0; y <= changeSizeY; y++){
        for(int x = 0; x <= changeSizeX; x++){
            ...
            drawScaledPixelsWindow(x, y, *((unsigned int*)(fb + oldOffs)));
