...
static int CURRENT_COLOR = white; //The color of the text to be drawn
...
void rotate(char* params){
    ...
    if(rotation < 0 || rotation > 4){
        printText("Rotation is out of bounds, must be between 0 and 4\n", CURRENT_COLOR);
        return;
    }
...
void scale(char* params){
    ...
    if(multiplication < 1 || multiplication > 5){
        printText("Multiplication is out of bounds, must be between 1 and 5\n", CURRENT_COLOR);
        return;
    }
...
void handleCommand(){
    ...
    //todo skip first space characters
    ...
    char programName[spaceIndex-inputStartIndex];
    ...
    char params[arrayGetLength(currentLine)-(spaceIndex+1)];
